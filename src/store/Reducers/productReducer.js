...
export const get_discount_products = createAsyncThunk(
    ...
    async (_, { rejectWithValue, fulfillWithValue }) => {
      try {
        const { data } = await api.get('/discount-products', { withCredentials: true });
        return fulfillWithValue(data);
      } catch (error) {
        return rejectWithValue(error.response?.data?.message || 'Failed to fetch discount products');
      }
    }
  );
//end method 
    ...
    extraReducers : (builder) => {
        ...
        .addCase(deleteProduct.pending, (state) => {
          ...
          .addCase(deleteProduct.fulfilled, (state, action) => {
            ...
            // bisa juga hapus produk dari state.products kalau perlu
          })
