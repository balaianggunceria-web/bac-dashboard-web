...
const Products = () => {
    ...
    const handleDelete = async (id) => {
        if (window.confirm('Are you sure you want to delete this product?')) {
          try {
            ...
            dispatch(get_products({ parPage, page: currentPage, searchValue }));
          } catch (error) {
    ...
    return (
        <div className='px-2 lg:px-7 pt-5'>
            <h1 className='text-[#000] font-semibold text-lg mb-3'>All Product</h1>
                        ...
                        <tbody>
                            {
                               products.map((d, i) => <tr key={i}>
                                ...
                                <img className='w-[50px] h-[50px]' src={d.images [0]} alt="" /></td>
                                ...
                                <td scope='row' className='py-1 px-4 font-medium whitespace-nowrap'>
                                    <div className='flex justify-start items-center gap-4'>
                                        ...
                                        <Link to={`/seller/dashboard/view-product/${d._id}`} className='p-[6px] bg-green-500 rounded hover:shadow-lg hover:shadow-green-500/50'> <FaEye/> </Link>
                                        <button className='p-[6px] bg-red-500 rounded hover:shadow-lg hover:shadow-red-500/50' onClick={() => handleDelete(d._id)}> <FaTrash/> </button> 
                                    </div>
