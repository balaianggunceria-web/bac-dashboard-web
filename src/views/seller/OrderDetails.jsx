                ...
                <div className='flex flex-wrap'>
                    <div className='w-[30%]'>
                        <div className='pr-3 text-[#d0d2d6] text-lg'>
                            ...
                            Deliver To : {order.shippingInfo?.name}, {order.shippingInfo?.address}, {order.shippingInfo?.city}, {order.shippingInfo?.province}, {order.shippingInfo?.post}
                            </div>
                            <div className='flex justify-start items-center gap-3'>
                                <h2>Payment Status : </h2>
                                <span className='text-base'>{order.payment_status}</span>
                            </div>
                            <span>Price : Rp.{order.price}</span> 

{
   order?.products?.map((p,i) => <div key={i} className='mt-4 flex flex-col gap-4 bg-[#8288ed] rounded-md'>
   <div className='text-[#d0d2d6]'> 
       <div className='flex gap-3 text-md'>
           <img className='w-[100px] h-[100px]' src={p.images[0]} alt="" />
